...
for pyfile in jovian-01-upstream-qc.py jovian-02a-downstream-qc.py jovian-02b-downstream-integrated.py jovian-03-DE.py; do
    ...
    jupyter nbconvert $output_folder/$ipynbfile --output ${ipynbfile/.ipynb/.html} --output-dir $reports_folder
    jupyter nbconvert $output_folder/$ipynbfile --output ${ipynbfile/.ipynb/.pdf} --output-dir $reports_folder
done
